...
if DEFAULT_ALGO == 'ddpg':
    DEFAULT_CPU = 1
else:
    DEFAULT_CPU = 4
DEFAULT_STEPS = 1 * int(1e6)
...
def run(
    ...
    init = np.array([[0,0,1]])
    prop_loss = 0
    sa_env_kwargs = dict(aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS, obs=obs, act=act,initial_xyzs=init,prop_fault=prop_loss)
